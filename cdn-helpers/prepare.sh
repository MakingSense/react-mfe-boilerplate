...
print_help () {
    ...
    echo ""
    echo "Prepare the files from ${dist} folder with the required structure to upload the files to our CDN into the folder ${ready}"
    echo ""
    echo "Options:"
...
mv "${dist}/asset-manifest.json" "${ready}/asset-manifest-${canonicalTag}.json"

cp -R "${dist}" "${ready}/dist"
